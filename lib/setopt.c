                        ...
                        va_list param)
{
  ...
  /* ---- Nintendo nn::ssl specific options ---- */
  case CURLOPT_SSL_SKIP_DEFAULT_VERIFY:
    /* Skip nn::ssl's default certificate verification */
    data->set.ssl.primary.skipssldefaultverify =
      (0 != va_arg(param, long)) ? TRUE : FALSE;
    if(data->conn)
      data->conn->ssl_config.skipssldefaultverify =
        data->set.ssl.primary.skipssldefaultverify;
    break;
  case CURLOPT_SSL_VERIFYDATE:
    data->set.ssl.primary.verifydate =
      (0 != va_arg(param, long)) ? TRUE : FALSE;
    if(data->conn)
      data->conn->ssl_config.verifydate = data->set.ssl.primary.verifydate;
    break;
  case CURLOPT_SSL_CONTEXT:
    /* application-supplied nn::ssl::Context* */
    data->set.ssl.psslctx = va_arg(param, void *);
    break;
  case CURLOPT_SSL_CONN_FUNCTION_BEFORE:
    data->set.ssl.fsslconnbefore =
      va_arg(param, curl_ssl_conn_callback_before);
    break;
  case CURLOPT_SSL_CONN_FUNCTION_AFTER:
    data->set.ssl.fsslconnafter =
      va_arg(param, curl_ssl_conn_callback_after);
    break;
  case CURLOPT_SSL_CONN_DATA:
    data->set.ssl.psslcondata = va_arg(param, void *);
    break;
  case CURLOPT_DISABLE_CACHE:
    /* accepted; DNS cache control handled by DNS_CACHE_TIMEOUT */
    (void)va_arg(param, long);
    break;
  case CURLOPT_PROXYAUTOCONFIG:
    /* accepted; system proxy auto-config is not supported in this port */
    (void)va_arg(param, long);
    break;
